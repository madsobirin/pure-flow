      ...
      <div className="w-full max-w-md bg-white min-h-screen relative shadow-sm flex flex-col md:my-8 md:rounded-[40px] md:min-h-[850px] md:shadow-xl md:border md:border-gray-100 overflow-hidden">
        <div className="flex-1 overflow-y-auto px-6 pb-28">
          ...
          <div className="space-y-4">
            {equipments.map((eq) => (
              <div key={eq.id} className="flex items-center gap-4 bg-white border border-[#eef2f6] rounded-[24px] p-4 shadow-sm">
                <div className="relative w-16 h-16 rounded-2xl overflow-hidden shrink-0">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={eq.image} alt={eq.name} className="w-full h-full object-cover" />
                </div>
                <div>
                  <h3 className="font-bold text-gray-900 text-[17px] mb-1">{eq.name}</h3>
                  <p className="text-gray-500 text-[13px] leading-tight line-clamp-2">{eq.notes}</p>
                </div>
              </div>
            ))}
          </div>
