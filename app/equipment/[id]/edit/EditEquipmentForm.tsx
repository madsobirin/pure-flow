...
export default function EditEquipmentForm({ alat }: EditEquipmentFormProps) {
  ...
  const [catatanAlat, setCatatanAlat] = useState(alat.catatan_alat || "");
  const [fotoFile, setFotoFile] = useState<File | null>(null);
  const [previewUrl, setPreviewUrl] = useState<string>(alat.foto_path);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      // Revoke old object URL if it was created dynamically
  ...
  const handleRemoveImage = () => {
    ...
    // Balikkan ke foto asli jika ada
    setPreviewUrl(alat.foto_path);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(result.message || "Gagal memperbarui alat.");
        setIsLoading(false);
      }
    } catch (err) {
    ...
    <>
      <form onSubmit={handleSubmit}>
        <div className="flex items-center gap-4 mb-6 mt-2">
          ...
          <h1 className="text-[19px] font-bold text-[#1a2332]">Edit Alat</h1>
        </div>
      ...
      <AnimatePresence>
        {showSuccessOverlay && (
          <SuccessOverlay message="Perubahan informasi alat berhasil disimpan." />
        )}
      </AnimatePresence>
    </>
  );
